...
DOCUMENTATION = r"""
---
module: get_service_bios_attributes
description: This module will get service BIOS settings from the HPE iLO server
requirements:
    - "python >= 3.6"
    - "ansible >= 2.11"
author:
...
RETURN = r"""
  expected_result:
    description: Service BIOS attributes received from the server
    returned: Service BIOS attributes present in the server
    type: dict
  failure case 1:
...
def main():
    ...
    service_attributes = get_service_bios_attributes(redfishClient, module)
    logout(redfishClient, module)
    module.exit_json(changed=False, service_attributes=service_attributes)
